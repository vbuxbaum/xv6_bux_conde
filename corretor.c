...
// testa o fork normal. garante que nÃ£o quebramos nada
// N forks e waits consecutivos. existe um limite de 64
// processos no xv6, entÃ£o temos que garantir que N <= 64
int caso1fork(void) {
  int n;
  int pid;
...
// mesmo do caso 1 sÃ³ que com o novo forkcow
// testa o fork cow. garante que nÃ£o quebramos nada
// N forks e waits consecutivos. existe um limite de 64
// processos no xv6, entÃ£o temos que garantir que N <= 64
...
// embora durante o forkcow algumas pÃ¡ginas sÃ£o escrita, os
// 2 processos nÃ£o crescem nem em pilha nem em heap.
// pipes, wait e exit para comunicaÃ§Ã£o e sync
int caso3numpgs(void) {
  ...
  int pid = forkcow();
  if (pid == 0) { // child manda nÃºmero de pÃ¡ginas de da exit
    ...
    close(fd[1]);
    exit();
  } else { // parent espera filho acabar e lÃª o fd
    close(fd[1]);
    wait();
...
// apÃ³s um write em uma GLOBAL. usamos a GLOBAL1_RW
// pipes, wait e exit para comunicaÃ§Ã£o e sync
int caso6cow(void) {
  int fd[2];
  pipe(fd);
...
int main(int argc, char *argv[]) {
  ...
  get_date(&r);
  print_date(&r);
  printf(stdout, "[Caso 3] Testando se o nÃºmero de pÃ¡ginas Ã© igual\n");
  call_ok = caso3numpgs();
  if (call_ok == FALSE) {
  ...
  get_date(&r);
  print_date(&r);
  printf(stdout, "[Caso 4] Testando se o endereÃ§o de uma constante Ã© =\n");
  ...
  get_date(&r);
  print_date(&r);
  printf(stdout, "[Caso 5] Testando se o endereÃ§o de uma global Ã© =\n");
